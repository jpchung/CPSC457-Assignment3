...
/*decrementing customer variable*/
int customers_left;
...
pthread_mutex_t remainder_mutex;
...
void *customer_run(void *arg) {
	...
	/*update customer generation until overtakes barista cleanup rate*/
	while(1) {
	/* Run for as long as we have customers incoming */
		...
		/*sleep for 10 seconds, then reset with new customer rate*/
		sleep(10);
		remaining_customers += customer_total;

		/*increase rate of customers - lock until complete*/
		pthread_mutex_lock(&remainder_mutex);

		customers_left += customer_total;

		pthread_mutex_unlock(&remainder_mutex);
	}
...
void *cleanup_run(void *arg) {
	...
	/*attempt to keep up with customer generation rate*/
	while (customers_cleared < customers_left) {
...
int main(int argc, char **argv) {
	...
	if((pthread_mutex_init(&remainder_mutex, NULL))!= 0) {
		printf("error: pthread_mutex_init\n");	
		return EXIT_FAILURE;
	}
	...
	pthread_mutex_destroy(&remainder_mutex);
